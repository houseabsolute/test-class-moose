...
Now, we need a subroutine that runs our test. C<Test::Class::Moose> test
...
Every C<test_> sub is run as a subtest, and no plan is required. We can have as
many C<test_> subs as we want.
...
loaded.
    ...
    use FindBin qw( $Bin );
    use Test::Class::Moose::Load qw( catdir( $Bin, 'lib', 'TestsFor' ) );
    Test::Class::Moose->new->runtests;

This test script will load all of the modules inside C<t/lib/TestsFor/> and
then run them. All your test modules get run by this one script, but since
they're run as subtests, you will get a report on how many test classes
failed.
...
To fix this problem, C<Test::Moose::Class> allows us to specify which specific
classes we want to run in its constructor:

    # t/test_class_tests.t
    use File::Spec::Functions qw( catdir );
    use FindBin qw( $Bin );
    use Test::Class::Moose::Load catdir( $Bin, 'lib', 'TestsFor' );
